...
#' 
#' This function is open access, but belongs to the openintro series.
#' They stopped hosting it within their data for their labs. As such,
#' I have gone ahead and recreated the variable below. I did not create
#' this function and do not own the rights to it. 
#'
#'
#' @export
inference <- function(y, x = NULL,
  ...
  if (x_type == "only1var") {
    ...
    if(est == "mean"){
      if(eda_plot == TRUE){hist(y, main = "", cex.main = 0.75, xlab = y_name, col = "red", ylab = "")}
      cat(paste("mean =", round(actual,4), "; ","sd =", round(sd(y), 4), "; ", "n =", n), "\n")
    }
    if(est == "median"){
      if(eda_plot == TRUE){
        boxplot(y, main = "", main = "", xlab = y_name, col = "red", axes = FALSE)
        axis(2)
      }
      cat(paste("median =", round(actual,4), "; ", "n =", n), "\n")
    }
    if(est == "proportion"){
      if(eda_plot == TRUE){barplot(table(y), main = "", xlab = y_name, col = "red")}
    ...
    if (method == "simulation") {
      ...
      if (type == "ci") {
        ...
        if(inf_plot == TRUE){
          ...
          if (nsim > 500) {
            ...
            hist(sim_dist, xlab = "Bootstrap distribution", main = "", ylab = "", xlim = xlim, col = "blue")
            if (inf_lines == TRUE) {
              for (i in 1:2) {
                segments(ci[i], 0, ci[i], 0.8 * max(counts), col="yellow", lwd=2)
                text(round(ci[i],4), max(counts), pos=1, col="yellow", round(ci[i],4))
              }
            }
          }
          if (nsim <= 500) {
            BHH2::dotPlot(sim_dist, xlim = xlim, pch=19, col="blue", cex = 0.8, axes = FALSE, xlab="Bootstrap distribution")
            axis(1)
            if(inf_lines == TRUE){
              for (i in 1:2) {
                segments(ci[i], 0, ci[i], 0.6, col="yellow", lwd=2)
                text(round(ci[i],4), 0.7, pos=1, col="yellow", round(ci[i],4))
              }
            }
          }
        }
      ...
      if (type == "ht") {
        ...
        if(inf_plot == TRUE){
          ...
          if (nsim > 500) {
            counts = hist(sim_dist, plot = FALSE)$counts
            hist(sim_dist, xlab = "Randomization distribution", main = "", ylab = "", xlim = xlim, col = "blue")
          }
          if (nsim <= 500) {
            BHH2::dotPlot(sim_dist, xlim = xlim, pch=19, col="blue", cex = 0.8, axes = FALSE, xlab="Randomization distribution")
            axis(1)
          }
        }
        ...
        if (alternative == "less") {
          ...
          if (inf_lines == TRUE) {
            if(nsim > 500) {lines(x = c(actual,actual), y = c(0,1.1*max(counts)), col="yellow", lwd=2)}
            if(nsim <= 500) {lines(x = c(actual,actual), y = c(0,0.8), col="yellow", lwd=2)}
          }
        }
        ...
        if (alternative == "twosided") {
          ...
          lines(x = c(actual,actual), y = c(0,1.1*max(counts)), col="yellow", lwd=2)
          if (actual >= null) {
            temp = actual - null
            if (inf_lines == TRUE) {
              if(nsim > 500) {lines(x = c(null - temp,null - temp), y = c(0,1.1*max(counts)), col="yellow", lwd=2)}
              if(nsim <= 500) {lines(x = c(null - temp,null - temp), y = c(0,0.8), col="yellow", lwd=2)}
            }
          }
          if (actual < null) {
            temp = null - actual
    ...
    if (method == "theoretical") {
      ...
      if (type == "ht") {
        ...
        if (alternative == "less") {
          if (actual < null) {cat(paste("p-value = ", smaller_tail,"\n"))}
          if (actual > null) {cat(paste("p-value = ", 1 - smaller_tail,"\n"))}
          openintro::normTail(L = teststat, axes = FALSE, col = "blue")
        ...
        if (alternative == "greater") {
          ...
          if (actual > null) {cat(paste("p-value = ", smaller_tail,"\n"))}
          if(inf_plot == TRUE){
            openintro::normTail(U = teststat, axes = FALSE, col = "blue")
        ...
        if (alternative == "twosided") {
          ...
          if (inf_lot == TRUE){
            if (actual < null) {
              openintro::normTail(L = teststat, U = -1*teststat, axes = FALSE, col = "blue")
              axis(1, at = c(-3, teststat, 0, -1*teststat, 3), labels = c(NA, paste(round(actual,2)), paste(null), paste(round(null + (null - actual), 2)), NA))
            }
            if (actual > null) {
              openintro::normTail(L = -1*teststat, U = teststat, axes = FALSE, col = "blue")
  ...
  if (x_type == "categorical") {
    ...
    if (x_levels == 2 & (y_type == "numerical" | (y_type == "categorical" & y_levels == 2))) {
      ...
      if (est == "mean") {
        ...
        if(eda_plot == TRUE){boxplot(y ~ x, xlab = x_name, ylab = y_name, main = "", col = "red")}
      }
      if (est == "median") {
        for(i in 1:x_levels) {
          cat("n_", names(by(y, x, length))[i], " = ", by(y, x, length)[i], ", ", sep="")
      ...
      if (est == "proportion") {
        ...
        if(eda_plot == TRUE){mosaicplot(table(x, y), xlab = x_name, ylab = y_name, main = "", col = "red")}
      }
      ...
      if (method == "simulation") {
        ...
        if (type == "ht") {
          ...
          if (alternative == "twosided") {
            ...
            if (actual >= null) {
              temp = actual - null
              if (inf_lines == TRUE) {lines(x = c(null - temp,null - temp), y = c(0,1.1*max(counts)), col = "blue", lwd=2)}
            }
            if (actual < null) {
              temp = null - actual
              if (inf_lines == TRUE) {lines(x = c(null + temp,null + temp), y = c(0,1.1*max(counts)), col = "blue", lwd=2)}
    ...
    if (x_levels > 2 & y_type == "numerical") {
      ...
      if(method == "theoretical"){
        ...
        if(inf_plot == TRUE) {FTail(F,df_n,df_d, col = "pink")}
      }
    ...
    if ((x_levels > 2 | y_levels > 2) & y_type == "categorical") {
      ...
      y_table = table(y, x)
      print(addmargins(y_table))
      mosaicplot(table(x, y), main = "", col = "red")
      cat("\n")
